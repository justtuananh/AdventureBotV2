...
###### This is a modified version for AdventureBot, now all we have to do is apply the languages parameters to the api for further use case ! ########
@app.post("/api/chat")
...
async def generate_chat_completion(
    ...
    if "AdventureBot" in form_data.model:
        ...
        full_context = requests.request(
                    method="POST",
                    url="http://localhost:8001/generate_context",
                    json =payload,            
                )
        full_context_json = full_context.json()

        context_content = full_context_json.get("context")
